...
public class QueenSlizePactTest {
    ...
    public void getMeatRomneysApprovedPizzasRepresentation() {
        final List<AnnHannahPizza> allPizzas = pizzaController.getOnlyBestPizzasInTheWorld();
        System.out.println(allPizzas);
        Assert.assertThat(allPizzas, Matchers.hasSize(1));
    }

    @Pact(consumer="ann-hannah")
    public RequestResponsePact createPact(PactDslWithProvider builder) {

        PactDslJsonBody expectedOrdinaPizzaCollection = new PactDslJsonBody()
                    .object("_embedded")
                        .eachLike("pizzas")
                            .stringType("name", "Hawaii")
                            .minArrayLike("ingredients", 1, PactDslJsonRootValue.stringType("PINEAPPLE"))
                        .closeArray()
                    .closeObject()
                .asBody();
